       ...
       if($Algorithm -eq $($Pools.(Get-Algo($_)).Coin))
        ...
        [PSCustomObject]@{
	        MinerName = "ethdcrminer64"
                ...
                Distro =  $Distro
                Devices = $Devices
                Arguments = "-mode 1 -mport 3333 -allpools 1 -allcoins 1 -epool $($Pools.(Get-Algo($_)).Host):$($Pools.(Get-Algo($_)).Port) -ewal $($Pools.(Get-Algo($_)).User1) -epsw $($Pools.(Get-Algo($_)).Pass1) $($Commands.$_)"
