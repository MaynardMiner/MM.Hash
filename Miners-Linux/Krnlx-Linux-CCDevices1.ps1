$Path = ".\Bin\krnlx-Linux-CCDevices1\ccminer-NVIDIA1"
 ...
 if($Algorithm -eq "$($AlgoPools.(Get-Algorithm($_)).Algorithm)")
  {
    ...
    Devices = $Devices
    DeviceCall = "ccminer"
    Arguments = "-a $(Get-Algorithm($_)) -o stratum+tcp://$($AlgoPools.(Get-Algorithm($_)).Host):$($AlgoPools.(Get-Algorithm($_)).Port) -b 0.0.0.0:4068 -u $($AlgoPools.(Get-Algorithm($_)).User1) -p $($AlgoPools.(Get-Algorithm($_)).Pass1) $($Commands.$_)"
