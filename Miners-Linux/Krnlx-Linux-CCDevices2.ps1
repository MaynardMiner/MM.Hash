$Path = ".\Bin\krnlx-Linux-CCDevices2\ccminer-NVIDIA2"
 ...
 if($Algorithm -eq "$($AlgoPools.(Get-Algorithm($_)).Algorithm)")
  {
    ...
    Devices = $Devices
    DeviceCall = "ccminer"
    Arguments = "-a $(Get-Algorithm($_)) -o stratum+tcp://$($AlgoPools.(Get-Algorithm($_)).Host):$($AlgoPools.(Get-Algorithm($_)).Port) -b 0.0.0.0:4069 -u $($AlgoPools.(Get-Algorithm($_)).User2) -p $($AlgoPools.(Get-Algorithm($_)).Pass2) $($Commands.$_)"
