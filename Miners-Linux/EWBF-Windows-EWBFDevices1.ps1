$Path = ".\Bin\EWBF-Windows-EWBFDevices1\miner"
  ...
  if($Algorithm -eq "$($Pools.(Get-Algorithm($_)).Algorithm)")
  {
    ...
    $Pools.PSObject.Properties.Value | Where-Object {$Commands."$($_.Algorithm)" -ne $null} | ForEach {
      ...
      [PSCustomObject]@{
        ...
        Devices = $Devices
        DeviceCall = "ewbf"
        Arguments = "--algo 192_7 --pers ZERO_PoW --api 0.0.0.0:42001 --server $($_.Host) --port $($_.Port) --user $($_.User1) --pass $($_.Pass1) $($Commands.$($_.Algorithm))"
