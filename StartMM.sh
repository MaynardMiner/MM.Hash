...
Interval='60'				                                  #Seconds Miner Runs Before Switching (5 minutes is reccommended, 10+ for slower devices)
...
PoolName='zergpool'			       	                          #Name of Pool You Wish To Mine In (See "Pools" dir To See Available Pools)
Type='"Nvidia","CPU"'				        	                  #Devices You Are Mining With (Currently Ony CPU)
Algorithm='"yescrypt","yescryptR16"'               #Algorithms You Wish To Mine...If Not Listed Here, MM.Hash Will Not Search Pool For It
...
o=$Proxy
#
pwsh -command "&.\MM.Hash.ps1 -StatsInterval $a -Username $b -WorkerName $c -RigName $d -Currency $e -Passwordcurrency $f -Interval $g -Delay $h -Wallet $i -Location $j -Poolname $k -Type $l -Algorithm $m -Donate $n -Proxy $o | Out-Host"
