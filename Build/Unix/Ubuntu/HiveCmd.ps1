...
function Get-AlgorithmList {
 ...
 function Start-LaunchCode {
        ...
        "
        Start-Process "killall.sh" -ArgumentList "$($Type)" -Wait    
        ...
        Start-Sleep -S 1
        if($Type  -like '*NVIDIA*'){$PreStart = Start-Process ".\pre-start.sh" -ArgumentList "$($Type) $Export"}
        Start-Sleep -S 1
        Write-Host "Starting $($Name) Mining $($Coins) on $($Type)" -ForegroundColor Cyan
        $NewMiner = Start-Process ".\startup.sh" -ArgumentList "$($Type) $CmdDir"
