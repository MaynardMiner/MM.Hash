       ...
       if($Algorithm -eq $($Pools.(Get-Algo($_)).Coin))
        ...
        [PSCustomObject]@{
            MinerName = "EthDcrMiner64"
                ...
                Distro =  $Distro
                Devices = $Devices
                Arguments = "-mport 3335 -mode 1 -esm 3 -estale 0 -allpools 1 -epool $($Pools.(Get-Algo($_)).Host):$($Pools.(Get-Algo($_)).Port) -ewal $($Pools.(Get-Algo($_)).User3) -epsw $($Pools.(Get-Algo($_)).Pass3) $($Commands.$_)"
