...
$Path = ".\Bin\NVIDIA-EWBF-GPUDevices2\miner.exe"
...
$Algorithms | Get-Member -MemberType NoteProperty | Select -ExpandProperty Name | ForEach {
	...
	Distro =  $Distro
	Devices = $Devices
        Arguments = "--api 0.0.0.0:42002 --server $($Pools.(Get-Algorithm($_)).Host) --port $($Pools.(Get-Algorithm($_)).Port) --fee 0 --solver 0 --eexit 1 --user $($Pools.(Get-Algorithm($_)).User2) --pass $($Pools.(Get-Algorithm($_)).Pass2)$($Optimizations.$_)"
